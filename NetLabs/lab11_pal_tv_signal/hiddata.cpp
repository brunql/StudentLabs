...
int usbhidOpenDevice(usb_dev_handle **device, int vendor, char *vendorName, int product, char *productName, int _usesReportIDs)
{
struct usb_bus      *bus;
...
void    usbhidCloseDevice(usb_dev_handle *device)
{
    if(device != NULL)
        usb_close( (usb_dev_handle *)device);//(void *)device);
}

/* ------------------------------------------------------------------------- */

int usbhidSetReport(usb_dev_handle *device, char *buffer, int len, int timeout)
{
int bytesSent;
...
int usbhidGetReport(usb_dev_handle *device, int reportNumber, char *buffer, int *len)
{
int bytesReceived, maxLen = *len;

    bytesReceived = usb_control_msg(device, USB_REQ_GET_STATUS, USBRQ_HID_GET_REPORT, USB_HID_REPORT_TYPE_FEATURE << 8, 0, buffer, maxLen, 1000);
    if(bytesReceived < 0){
        fprintf(stderr, "Error sending message: %s\n", usb_strerror());
        return USBOPEN_ERR_IO;
    }
